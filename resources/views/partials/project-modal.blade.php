<div
    ...
    x-transition:leave="animate-fadeOut" {{-- (Butuh keyframe fadeOut) --}}
    class="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-6"
    style="display: none;" {{-- Sembunyikan awalnya --}}

    {{-- 2. Kalau klik area gelap, tutup modal (set jadi null) --}}
    @click="selectedProject = null"
>
    <div
        x-show="selectedProject"
        x-transition:enter="animate-modalIn"
        x-transition:leave="animate-modalOut"
    ...
    >
        <div class="flex justify-between items-center p-5 border-b border-gray-800">
            <h3
                ...
                x-text="selectedProject ? selectedProject.judul_proyek : ''"
            ></h3>
            <button
                @click="selectedProject = null"
        ...
        <div class="p-6 max-h-[70vh] overflow-y-auto">
            <img
                {{-- 5. Kita bind 'src' gambarnya pakai x-bind --}}
                :src="selectedProject ? '{{ asset('') }}/' + selectedProject.gambar_url : ''"
            ...
            <div class="mb-6">
                ...
                <p
                    ...
                    x-text="selectedProject ? selectedProject.deskripsi_proyek : ''"
                ></p>
            </div>
...
<div class="mb-6">
    ...
    <div class="flex flex-wrap gap-2">
        <template x-if="selectedProject && selectedProject.tags" x-for="tag in selectedProject.tags.split(',')" :key="tag">
            <span
                class="bg-gray-800 text-brand-purple text-sm font-medium px-3 py-1 rounded-full"
                x-text="tag"
            ></span>
        </template>
    </div>
            ...
            <div class="flex flex-col sm:flex-row gap-4">
                ...
                <template x-if="selectedProject && selectedProject.link_toko">
                    <a
                        :href="selectedProject.link_toko"
                        target="_blank"
                        rel="noopener noreferrer"
